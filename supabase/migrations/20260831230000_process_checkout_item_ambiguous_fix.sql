-- Fix: "column reference item is ambiguous" in process_checkout stock aggregation.
-- PL/pgSQL variable `item` conflicted with SQL alias `item` in jsonb_array_elements subquery.
...
BEGIN
  ...
  FOR v_product_id, v_required_qty, v_product_name IN
    SELECT
      (cart_line.value->>'product_id')::uuid,
      SUM((cart_line.value->>'quantity')::numeric),
      MAX(p.name)
    FROM jsonb_array_elements(p_items) AS cart_line(value)
    JOIN public.products p ON p.id = (cart_line.value->>'product_id')::uuid AND p.business_id = p_business_id
    GROUP BY (cart_line.value->>'product_id')::uuid
  LOOP
  ...
  LOOP
    v_product_id := (cart_item->>'product_id')::uuid;
    v_qty := (cart_item->>'quantity')::numeric;
    v_is_wholesale := COALESCE((cart_item->>'is_wholesale_rate')::boolean, false);
  ...
  LOOP
    INSERT INTO public.sale_items (
      ...
      (cart_item->>'quantity')::numeric,
      (cart_item->>'unit_price')::numeric,
      (cart_item->>'subtotal')::numeric,
      (cart_item->>'tax_rate')::numeric,
      (cart_item->>'tax_amount')::numeric,
      (cart_item->>'tax_inclusive')::boolean,
      COALESCE((cart_item->>'is_wholesale_rate')::boolean, false)
    );
