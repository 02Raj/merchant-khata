-- Wholesale fixes: persist WS rate on bills, server-side MOQ + credit limit enforcement

ALTER TABLE public.sale_items
  ADD COLUMN IF NOT EXISTS is_wholesale_rate boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  LOOP
    ...
    SELECT COALESCE(SUM(it.quantity_change), 0), p.name, p.moq
    INTO v_stock, v_product_name, v_moq
  ...
  IF v_credit > 0 AND p_customer_id IS NOT NULL THEN
    ...
    IF v_credit_limit IS NOT NULL AND v_credit_limit > 0 THEN
      ...
      END IF;
    ELSIF v_business_type IN ('wholesale', 'both') THEN
      -- Wholesale shops: unlimited udhaar when no limit configured (client may warn)
      NULL;
    END IF;
  END IF;
